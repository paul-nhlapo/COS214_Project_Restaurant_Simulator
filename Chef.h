...
class Chef
{
    public:
        ...
        virtual void prepareMeal(Order* order);
        ...
        virtual void receiveNotification(Chef* senderChef) = 0;         //Recieve Notification that another Chef has changed its STATE                   
                                                                        //Each Chef must implement it differently

        virtual void addOrderToQueue(Order* order) {};
